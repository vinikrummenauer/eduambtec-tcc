...
var infinityGround = {
    ...
    moveBg() {
        ...
        if (this.ground[0].x <= -500) {
            this.ground[0].x = 0;
        }
        if (this.ground2[0].x <= 0) {
            this.ground2[0].x = 500;
        }
    },
}
...
var game = {
    ...
    update() {
        ...
        if(score >= 3){
        pipe1[0].move(5.5, -100, 1800, pipe2);
        pipe3[0].move(5.5, -100, 1800, pipe2);
        pipe4[0].move(5.5, -100, 1800, pipe2);
        pipe5[0].move(5.5, -100, 1800, pipe2);
        }
        if(score >= 6){
        pipe1[0].move(6, -100, 1800, pipe2);
        pipe3[0].move(6, -100, 1800, pipe2);
        pipe4[0].move(6, -100, 1800, pipe2);
        pipe5[0].move(6, -100, 1800, pipe2);
        }
...
function acertarChao() {
    groupShoot.forEach((shoot) => {
        infinityGround.ground.forEach((ground) => {
            if (shoot.collide(ground)) {
                score -= 1;
            }
        });
    });
    groupShoot.forEach((shoot) => {
        infinityGround.ground2.forEach((ground2) => {
            if (shoot.collide(ground2)) {
                score -= 1;
            }
        });
    });
